...
function renderAbout() {
  ...
  [
    ...
    { icon: '📖', title: 'הלומדה — הסיכום המלא, בשלושה מצבים',
      body: 'לכל מקצוע יש לומדה: סיכום מלא עם איורים, סרטונים, הקראה ותרגילים. במתג שבתחתית בוחרים ' +
            'איך לקרוא אותה: 📖 קריאה מלאה — כל התוכן, כמעבר ראשון על החומר; ⚡ מרוכז — רק התמצית, ' +
            'המלכודות ו"מה באמת נשאל" בכל נושא, לחזרה מהירה לפני מבחן; 🎮 אינטראקטיבי — תרגילי התאמה, ' +
            'מפות חשיבה שנפתחות, ושערי "נסה קודם" שנותנים לך לענות לפני שהם חושפים. אפשר גם לסמן טקסט ' +
            'במרקר (נשמר לפעם הבאה), ובסוף כל נושא — קפיצה ישירה לתרגול שלו.' },
    { icon: '🗺️', title: 'מפת החומרים — מה ללמוד ומה באמת נשאל',
      body: 'המפה מסדרת את הנושאים בשתי דרכים: לפי סדר הלימוד (כמו שלימדו, החומר בנוי אחד על השני) ' +
            'או לפי נפח במבחן (הכי נשאל קודם). לכל נושא — מאיפה ללמוד, ומה נשאל עליו בפועל בשחזורים, ' +
            'עם קישור לכל שאלה. כל נושא מקשר ישר לתרגול עליו.' },
...
function unitCard(courseId, g, r, focus, collapsible) {
  ...
  const sd = courseOf(courseId) && courseOf(courseId).studyDoc;
  if (sd) {
