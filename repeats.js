...
const COURSES = {
  molecular: {
    ...
    /* "שחזור מחזור נ״א — מועד א׳" → "נ״א"  (ואם זה מועד ב׳: "נ״א ב׳").
       מבחן שאינו של מחזור רפואה (מעבדה/רוקחות — אין "מחזור X" בכותרת): גוזרים שם
       קריא מהכותרת עצמה במקום מספר ה-cycle הגולמי, כדי שהתג לא יציג "52". */
    label: (e) => {
      const m = (e.title || '').match(/מחזור\s+([^\s—–]+)/);
      const base = m
        ? m[1]
        : (e.title || String(e.cycle ?? e.id)).replace(/^שחזור\s+/, '').split(/[—–]/)[0].trim();
      return e.moed === 'ב' ? `${base} ב׳` : base;
    },
  },
  /* שני קורסי הבלוק של שנה ב׳. הציר הוא מחזור, כמו בביומול — מחזור נ״א=51,
     נ״ב=52, ואחורה (מ״ט=49, נ׳=50). המבחנים הישנים שמיובאים לכאן הם של
     הקורסים הנפרדים שקדמו לתוכנית, ולכן הכותרת נושאת גם את שם המקצוע. */
  'ekronot-a': {
    axis: 'cycle',
    part: null,
    unit: 'מחזורים',
...
if (process.argv.includes('--ask')) {
  ...
  const COURSE_NAME = { molecular: 'ביולוגיה מולקולרית', electro: 'אלקטרופיזיולוגיה',
    'ekronot-a': 'עקרונות המדע הבסיסיים א׳', 'ekronot-b': 'עקרונות המדע הבסיסיים ב׳' };
  const prompt = [
